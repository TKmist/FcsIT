...
# Directory containing correlation files from one simulation dataset.
CORRELATION_DIR="${1:-/path/to/simulation_correlations/AutoCorr_ch1}"
...
fit_format() {
    ...
    fcsit_call fitting.set_parameter \
        '{"name":"kappa","value":5.25,"fixed":true}'
    fcsit_call fitting.set_parameter \
        '{"name":"tau_d1","value":0.1,"maximum":0.25}'
...
echo "Completed simulation fitting: ${CORRELATION_DIR}"
